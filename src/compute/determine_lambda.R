...
path_prefix <- "designA_T500_p50" # path_prefix <- args[1]
sim_id_dir <- "" # sim_id_dir <- args[2]
...
sim_id_dir <- file.path(PROJ_DIR, sim_id_dir)
...
path_y <- paste0(data_dir, sim_id_dir, path_prefix, "_y.csv")
...
# Calculate lambda_0 for the GSPLASH
lam0_reg_a0 <- calc_lambda_0(sigma_hat, Vhat_d, reg_gr, alpha = 0, verbose = TRUE)
...
df_lam_reg_a0 <- create_lambda_df(grid_lam_reg_a0, file.path(lambdas_dir, sim_id_dir, "reg_a0.csv"))
df_lam_reg_a05 <- create_lambda_df(grid_lam_reg_a05, file.path(lambdas_dir, sim_id_dir, "reg_a05.csv"))
df_lam_sym_a0 <- create_lambda_df(grid_lam_sym_a0, file.path(lambdas_dir, sim_id_dir, "sym_a0.csv"))
df_lam_sym_a05 <- create_lambda_df(grid_lam_sym_a05, file.path(lambdas_dir, sim_id_dir, "sym_a05.csv"))
