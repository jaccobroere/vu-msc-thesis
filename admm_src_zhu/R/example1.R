			...
			p <- num.groups * num.vars.per.group
                for (cor in c(.5,.7,.9)){
                        ...
                        out <- linreg_path(data[[1]],data[[2]],data[[3]],data[[4]],data[[5]],
							lambda_graph,lambda_sparse,data[[6]],data[[7]],variantADMM=variant)
                        time.diff <- proc.time() - ptm
                        summary <- c(time.diff,mean(out$admm_iter_num),sum(out$chol_num))
                        filename = paste("variant",toString(variant),"groups",toString(num.groups),"cor",toString(cor),"accelerated.RDtata",sep="")
