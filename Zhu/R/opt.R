...
linreg_path <- function(Y, X, val, idx, jdx, lambda_graph,
    ...
    if (nonadaptive_varying_rho && constant_rho) {
        stop("nonadaptive_varying_rho and constant_rho can't both be TRUE! \n")
    }
    if (constant_rho) {
        nonadaptive_varying_rho <- FALSE
        adaptive_varying_rho <- FALSE
    }
    if (nonadaptive_varying_rho) {
        constant_rho <- FALSE
        adaptive_varying_rho <- FALSE
    }
    special_ADMM <- TRUE
    if (standard_ADMM && linearized_ADMM) {
        stop("standard_ADMM and linearized_ADMM can't both be TRUE! \n")
    }
    if (standard_ADMM) {
        linearized_ADMM <- FALSE
        special_ADMM <- FALSE
    }
    if (linearized_ADMM) {
        standard_ADMM <- FALSE
    ...
    if (reporting) {
        out$fun_path <- array(out$fun_path, max_num_iter, c(length(lambda_sparse), length(lambda_graph)))
    }
    return(out)
}
...
linreg_path_v2 <- function(Y, X, val, idx, jdx, lambda_graph, gamma, p, m, rho = 1.0,
    ...
    if (reporting) {
        out$fun_path <- matrix(out$fun_path, max_num_iter, length(lambda_pairs) / 2)
    }
    return(out)
}
...
filtering <- function(Y, val, idx, jdx, lambda, m, rho = 1.0, eps = 1e-3, max_iter = 1e5,
    ...
    out <- .Call(
        "filtering", Y, val, as.integer(idx), as.integer(jdx), lambda, as.integer(m),
    ...
    if (reporting) {
        out$fun_path <- matrix(out$fun_path, max_iter, length(lambda))
    }
    return(out)
}
