...
Node* Graph::AddNode()
{
    ...
    node.label = 0; // Added by Y. Kawahara
...
Arc* Graph::AddArc(Node* tailNode, Node* headNode, double cap)
{
...
Arc* Graph::AddArcFromSource(Node* tailNode, Node* headNode, double cap, double slope)
{
...
Arc* Graph::AddArcToSink(Node* tailNode, Node* headNode, double cap, double slope)
{
	Arc* arc = AddArc(tailNode, headNode, cap);
...
{
	LongType minNom = std::numeric_limits<double>::max();
	LongType minDenom = 1;
...
{
	LongType maxNom = std::numeric_limits<double>::min();
	LongType maxDenom = 1;
...
void Graph::SetMultiplier(double multiplier)
{
...
double Graph::ChooseMultiplier(LongType lambda1, LongType lambda2)
{
	LongType maxEx = 0;
	for (NodeIterator node = nodes.begin(); node != nodes.end(); node++)
	{
	...
	return std::min(std::numeric_limits<LongType>::max()/2/maxEx,(std::numeric_limits<double>::max()/ 2));
}
