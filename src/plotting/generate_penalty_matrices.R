...
D_alpha <- rbind(D, diag(ncol(D)))
writeMM(D_alpha, file.path(data_dir, "D_alpha_GFSPLASH.mtx"))

D_sym <- as(getDg(graph = sym_gr), "TsparseMatrix")
# sym_part <- as(getDg(graph = sym_gr), "TsparseMatrix")
D_alpha_sym <- rbind(D_sym, diag(ncol(D_sym)))
writeMM(D_alpha_sym, file.path(data_dir, "D_alpha_sym_GFSPLASH.mtx"))
